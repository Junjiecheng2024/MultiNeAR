...
def resample_volume(volume, original_spacing, target_spacing, is_label=False):
    ...
    Args:
        volume: Input 3D array (D, H, W)
        original_spacing: Original voxel spacing (x, y, z) in mm
        target_spacing: Target resolution/spacing
            If values >= 10: treated as target voxel count, e.g., (128, 128, 128)
            If values < 10: treated as physical spacing in mm, e.g., (1.0, 1.0, 1.0)
        is_label: If True, use nearest neighbor interpolation (for labels)
                 If False, use cubic interpolation (for images)
    ...
    if all(s >= 10 for s in target_spacing):
        ...
        if is_label:
            resampled = sk_resize(
                volume, target_shape, order=0,
                preserve_range=True, anti_aliasing=False
            ).astype(volume.dtype)
        else:
            resampled = sk_resize(
                volume, target_shape, order=3,
                preserve_range=True
            ).astype(np.float32)
...
def normalize_ct(ct_array, hu_min=-1000, hu_max=1000, method="minmax"):
    """Clip CT intensities to HU range (no normalization).
    
    Note: Saves raw HU values after clipping. Normalization is left
    to the Dataset class for runtime flexibility.
    """
...
def main(config):
    ...
    print(f"  Failed: {len(failed_results)}/{len(case_ids)}")
    ...
    if failed_results:
        ...
        if len(failed_results) > 10:
            print(f"  ... {len(failed_results) - 10} more failed samples")
    ...
    if successful_results:
        ...
        print(f"\nSaved sample list: {info_path}")
        print(f"  Number of samples: {len(info_df)}")
    ...
    print("\nRecommended class weights (for training config):")
    ...
    print("=" * 80)
    print(f"\nNext steps:")
    print(f"  1. Check output directory: {output_root}")
    print(f"  2. Update training config data_path to: {output_root}")
    print(f"  3. Copy class_weights to config_near.py")
    print(f"  4. Run training: python near_repair.py")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seg_dir", type=str, help="Segmentation directory")
    parser.add_argument("--output_root", type=str, help="Output root directory")
    parser.add_argument("--target_resolution", type=str, help="Target resolution, e.g., '1.0,1.0,1.0'")
    parser.add_argument("--n_workers", type=int, help="Number of parallel processes")
    parser.add_argument("--hu_min", type=int, help="Minimum HU value")
    parser.add_argument("--hu_max", type=int, help="Maximum HU value")
