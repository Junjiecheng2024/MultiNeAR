"""Multi-core task execution utilities."""
...
class TaskRunner:
    ...
    def __init__(self, task, arg_list, max_workers=AVAILABLE_CORES // 2, use_tqdm=True):
        ...
        Args:
            ...
            arg_list: List of arguments to pass to task
    ...
    def run(self):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(self._task, arg) for arg in self.arg_list}
    ...
    def _task(self, arg):
        """Wrapper function executed in subprocess."""
        try:
            ret = self.task(arg)
    ...
    @property
    def errors_(self):
        """Get list of failed task arguments."""
...
if __name__ == '__main__':
    ...
    # Parallel execution
    now = time.time()
    runner = TaskRunner(task=task, arg_list=range(10), max_workers=5)
    runner.run()
